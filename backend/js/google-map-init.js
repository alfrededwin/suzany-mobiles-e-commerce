var ProjectX_googlemap_init_obj = {};
var ProjectX_GEOCODE_ERROR = "Error";
// Google map Styles
var ProjectX_googlemap_styles = {
    'default': [],
...
function googlemap_init(dom_obj, coords) {
    ...
    try {
        ...
        var id = dom_obj.id;
        ProjectX_googlemap_init_obj[id] = {};
        ProjectX_googlemap_init_obj[id].dom = dom_obj;
        ProjectX_googlemap_init_obj[id].point = coords.point;
        ProjectX_googlemap_init_obj[id].description = coords.description;
        ProjectX_googlemap_init_obj[id].title = coords.title;
        ProjectX_googlemap_init_obj[id].opt = {
            zoom: coords.zoom,
            center: latlng,
            scrollwheel: false,
            scaleControl: true,
            ...
            styles: ProjectX_googlemap_styles[coords.style ? coords.style : 'default'],
        ...
        custom_map.geocode(coords.latlng!=='' ? {'latLng': latlng} : {"address": coords.address}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                ProjectX_googlemap_init_obj[id].address = results[0].geometry.location;
                googlemap_create(id);
            } else {
                alert(ProjectX_GEOCODE_ERROR + ' ' + status);
            }
        });

        jQuery(window).resize(function() {
            if (ProjectX_googlemap_init_obj[id].map) {
                ProjectX_googlemap_init_obj[id].map.setCenter(ProjectX_googlemap_init_obj[id].address);
            }
        });
...
function googlemap_create(id) {
    "use strict";
    if (!ProjectX_googlemap_init_obj[id].address) { 
        return false;
    }
    ProjectX_googlemap_init_obj[id].map = new google.maps.Map(ProjectX_googlemap_init_obj[id].dom, ProjectX_googlemap_init_obj[id].opt);
    ProjectX_googlemap_init_obj[id].map.setCenter(ProjectX_googlemap_init_obj[id].address);
    var markerInit = {
        map: ProjectX_googlemap_init_obj[id].map,
        position: ProjectX_googlemap_init_obj[id].address   //ProjectX_googlemap_init_obj[id].map.getCenter()
    };
    if (ProjectX_googlemap_init_obj[id].point) { 
        markerInit.icon = ProjectX_googlemap_init_obj[id].point;
    }
    if (ProjectX_googlemap_init_obj[id].title) { 
        markerInit.title = ProjectX_googlemap_init_obj[id].title;
    }
    var marker = new google.maps.Marker(markerInit);
    var infowindow = new google.maps.InfoWindow({
        content: ProjectX_googlemap_init_obj[id].description
    });
    google.maps.event.addListener(marker, "click", function() {
        infowindow.open(ProjectX_googlemap_init_obj[id].map, marker);
    });
}


function googlemap_refresh(){
    for(id in ProjectX_googlemap_init_obj){
        googlemap_create(id);
    }
}
